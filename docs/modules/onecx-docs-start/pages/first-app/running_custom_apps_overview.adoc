...
After xref:./create_angular_app.adoc[creating a new application], we want to run it in our local environment. 
...
Before you begin, please ensure your local OneCX environment is already set up and running correctly. For setup instructions, refer to the xref:documentation:onecx-local-env:index.adoc[onecx-local-env documentation].
Additionally, make sure that entries for `hello-world-bff` and `hello-world-svc` are added to your system's hosts file (`C:\Windows\System32\drivers\etc\hosts` on Windows) the same way as it was done in the xref:documentation:onecx-local-env:setup.adoc[onecx-local-env setup].
...
* If you used the OneCX generator as outlined in the xref:./create_angular_app.adoc[Getting Started guide], you can quickly set up your application using the `onecx-local-env-cli`. See the detailed steps here: xref:./app_setup_cli.adoc[Application Setup using OneCX CLI]
