...
== Overview
`@onecx/angular-integration-interface` library is a set of tools aiming to ease the development of Angular applications integrated with OneCX by utilizing and extending functionalities of xref:integration-interface.adoc[@onecx/integration-interface]. This library offers Injectable services exposing a set of functionalities useful for integrating Microfrontends with OneCX without directly utilizing Topics.
...
|`getConfig()` |`xref:integration-interface.adoc#config_object[Config]` |Gets all configuration variables defined for the app from the topic.
|===
...
+
This service creates a new instance of xref:integration-interface.adoc#remote_components_topic[RemoteComponentsTopic] which contains information about the Remote Components and makes sure it is cleaned up correctly on destroy.
...
+
This service allows changing the currently used Theme by applying it to the document and informs about it via a new message in the xref:integration-interface.adoc#current_theme_topic[CurrentThemeTopic].
...
|`apply(theme: xref:integration-interface.adoc#theme_object[Theme])` | `Promise<void>` | Applies Theme via document style manipulation (styles will be lost on page exit). Publishes a new message via CurrentThemeTopic.
