...
* **Microfrontend**: A Microfrontend is an application exposing piece of user interface via a remote entry file. It is to be loaded in a host application instead of running in standalone. It is completely independent from other Microfrontends, hence the name. More about Microfrontends can be found xref:architecture-overview/mfe.adoc[here].
...
The Shell is considered as one of the Core Applications in the OneCX platform architecture. Since no Microfrontend can be used without the Shell application, it is the most important one. In order to serve correct content it needs to cooperate with other core applications, which are responsible for different parts of platform management. Those applications are:
