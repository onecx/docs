...
**Microfrontend**:: A Microfrontend is an application exposing a piece of user interface via a remote entry file. It is to be loaded in a host application instead of running in standalone. It is completely independent from other Microfrontends, hence the name. More about Microfrontends can be found xref:architecture-overview/mfe.adoc[here].
