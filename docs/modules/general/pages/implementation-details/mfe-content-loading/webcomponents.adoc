...
== Overview
Web Components is a suite of different technologies used to create reusable custom elements with encapsulated functionality. In OneCX context they are useful for dynamic content creation for both modules and Remote Components display of Microfrontends. Prior to reading this document it is recommended to read the documentation of xref:implementation-details/mfe-content-loading/index.adoc[microfrontend content loading] to get familiar with the content loading mechanisms available within the OneCX platform. 
