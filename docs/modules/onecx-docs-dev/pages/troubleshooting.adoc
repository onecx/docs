...
:page-aliases: guides/troubleshooting
...
[#onecx-compatibility]
== OneCX Compatibility
Make sure that the App is xref:documentation:onecx-portal-ui-libs:onecx-compatibility.adoc[OneCX compatible and you followed the guidelines].
...
In case the application results in "TypeError: Cannot read properties of undefined (reading 'ɵcmp')" after being loaded, the most common cause is that your project is using link:https://www.typescriptlang.org/tsconfig/#paths[typescript paths].

image::mappings_issue.png[]

After confirming, please consider updating your webpack.config to not include any sharedMappings

.webpack.config.js
----
const config = withModuleFederationPlugin({
  // existing module federation config
  sharedMappings: [] // <-- important change to resolve the issue
})
----
