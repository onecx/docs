...
## [](#why-assets-need-special-treatment)Why assets need special treatment in OneCX
...
### [](#translation-assets)Translation assets (required)
...
### [](#application-styles)Application styles (recommended)
...
Note the boundary with the **[Style Isolation](style-isolation.html)** concept: exposing `styles.css` is the **what** and **how** — the mechanics of publishing the file so the shell can load it. What **belongs** inside that stylesheet, and how it is scoped so it does not leak into or out of the application, is a separate concern owned by the **Style Isolation** concept page.
...
* [Style Isolation](style-isolation.html) — rules for what belongs in an application’s `styles.css` and how the platform scopes it.
