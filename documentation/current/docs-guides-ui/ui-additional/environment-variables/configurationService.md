...
## [](#configuration-service-configure-env-json)Configure `env.json`

The Shell has its own `assets/env.json` file.`ConfigurationService` loads its runtime configuration from that file.

Configure these values only by setting regular environment variables for the Shell deployment. The Shell UI container injects those values into `assets/env.json`.
...
## [](#configuration-service-access-variables)Access variables
