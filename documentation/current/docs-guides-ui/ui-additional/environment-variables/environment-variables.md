...
## [](#environment-variables-what-is-env-json)What is `env.json`?
...
| |  src/assets/env.json can be treated as a **deployment-time template**. It may contain placeholders like ${KEYCLOAK\_URL} that get replaced when the app is deployed (for example by a container entrypoint). |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

## [](#environment-variables-config-scopes)Config scopes

OneCX UI applications use two runtime config scopes:

* **MFE scope** (per microfrontend): [AppConfigService](../../../onecx-portal-ui-libs/libraries/angular-integration-interface.html#app-config-service)
* **Shell scope** (shared across multiple microfrontends): [ConfigurationService](../../../onecx-portal-ui-libs/libraries/angular-integration-interface.html#configuration-service)

Global values are shared across apps. Per-app values belong to one app only.

## [](#environment-variables-how-to-influence-values)How to influence values

### [](#environment-variables-app-values)App values (`AppConfigService`)
...
Further details are available in [AppConfigService](appConfigService.html).

### [](#environment-variables-shell-values)Shell values (`ConfigurationService`)
...
Further details are available in [ConfigurationService](configurationService.html).

## [](#environment-variables-files-configured)Files configured (Angular)
...
### [](#environment-variables-troubleshooting-env-json-404)`GET …​/assets/env.json` returns 404
...
### [](#environment-variables-troubleshooting-config-not-applied)Config is not applied
